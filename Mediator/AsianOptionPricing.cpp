/**
* Author: Zhixin Li
* File name: AsianOptionPricing.cpp
* Date: 2023.04.28
* Description: The implementaion file for the mediator for the Asian options.
* Version: 1.0
* */

#ifndef ASIANOPTIONPRICING_CPP
#define ASIANOPTIONPRICING_CPP
...
#include "AsianOptionPricing.hpp"
...
AsianOptionPricing<T, Container, Alloc>::AsianOptionPricing(const int& sims, const int& step, const double& init_condition) :
	Mediator<T, Container, Alloc>(sims, step, init_condition) {}

template<typename T, template<typename T, typename Alloc> class Container, typename Alloc>
AsianOptionPricing<T, Container, Alloc>::~AsianOptionPricing() {}

template<typename T, template<typename T, typename Alloc> class Container, typename Alloc>
const void AsianOptionPricing<T, Container, Alloc>::insertUserMonitor(Observers<T, Container, Alloc>& obs, const string& name, const int& cate)
{
...
template<typename T, template<typename T, typename Alloc> class Container, typename Alloc>
void AsianOptionPricing<T, Container, Alloc>::removeUser(const string& name, const int& cate)
{
	tuple<string, int> user_info = make_tuple(name, cate);
...
template<typename T, template<typename T, typename Alloc> class Container, typename Alloc>
void AsianOptionPricing<T, Container, Alloc>::start(shared_ptr<OptionInfo>& option_info, const int& sde_type, const int& rand_type, const int& fdm_type)
{
	...
	auto func = [&](int seed) {
		for (int j = 0; j < scale; j++)
		{
			...
			// Set Pricer
			this->pricer_pos = make_shared<AsianPricer<T, Container, Alloc>>(path1, this->option);
			T option_price1 = this->pricer_pos->pricing(); option_price += option_price1;
			this->pricer_neg = make_shared<AsianPricer<T, Container, Alloc>>(path2, this->option);
