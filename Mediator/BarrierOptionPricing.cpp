/**
* Author: Zhixin Li
* File name: BarrierOptionPricing.cpp
* Date: 2023.04.28
* Description: The implementaion file for the mediator for the Barrier options.
* Version: 1.0
* */

#ifndef BARRIEROPTIONPRICING_CPP
#define BARRIEROPTIONPRICING_CPP
...
#include "BarrierOptionPricing.hpp"
...
BarrierOptionPricing<T, Container, Alloc>::BarrierOptionPricing(const int& sims, const int& step, const double& init_condition) :
	Mediator<T, Container, Alloc>(sims, step, init_condition) {}

template<typename T, template<typename T, typename Alloc> class Container, typename Alloc>
BarrierOptionPricing<T, Container, Alloc>::~BarrierOptionPricing() {}

template<typename T, template<typename T, typename Alloc> class Container, typename Alloc>
const void BarrierOptionPricing<T, Container, Alloc>::insertUserMonitor(Observers<T, Container, Alloc>& obs, const string& name, const int& cate)
{
...
template<typename T, template<typename T, typename Alloc> class Container, typename Alloc>
void BarrierOptionPricing<T, Container, Alloc>::removeUser(const string& name, const int& cate)
{
	tuple<string, int> user_info = make_tuple(name, cate);
...
template<typename T, template<typename T, typename Alloc> class Container, typename Alloc>
void BarrierOptionPricing<T, Container, Alloc>::start(shared_ptr<OptionInfo>& option_info, const int& sde_type, const int& rand_type, const int& fdm_type)
{
	...
	int scale = 100; double option_price = 0;
	...
	auto func = [&](int seed) {
		for (int j = 0; j < scale; j++)
		{
			...
			// Set Pricer
			BarrierPricer<double, vector, allocator<double>> pricer_pos(path1, this->option, this->barrier_type, this->barrier);
			T option_price1 = pricer_pos.pricing(); option_price += option_price1;
			BarrierPricer<double, vector, allocator<double>> pricer_neg(path2, this->option, this->barrier_type, this->barrier);
...
template<typename T, template<typename T, typename Alloc> class Container, typename Alloc>
const void BarrierOptionPricing<T, Container, Alloc>::setBarrierType(const int& val)
{
	barrier_type = val;
}

template<typename T, template<typename T, typename Alloc> class Container, typename Alloc>
const void BarrierOptionPricing<T, Container, Alloc>::setBarrier(const double& val)
{
	barrier = val;
}
